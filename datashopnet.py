# -*- coding: utf-8 -*-
# Copyright 2019 jjunghub, Kakao Arena contest
#
# modified data.py Copyright 2017 Kakao, Recommendation Team
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""
# datashopnet.py
# written by jjung 2018.12.15
# raw dataset to pre-processed datset with word indexing.
"""

import os
os.environ['OMP_NUM_THREADS'] = '1'
import re
import sys
import traceback
from collections import Counter
from multiprocessing import Pool
import words as wd

import tqdm
import fire
import h5py
import numpy as np
import mmh3
import six
from keras.utils.np_utils import to_categorical
# from six.moves import cPickle
import pickle

from misc import get_logger, Option
opt = Option('./config.json')

# re_sc = re.compile('[\!@#$%\^&\*\(\)-=\[\]\{\}\.,/\?~\+\'"|]')
# re_sc = re.compile('[\W\d_]')


class Reader(object):
    def __init__(self, data_path_list, div, begin_offset, end_offset):
        self.div = div
        self.data_path_list = data_path_list
        self.begin_offset = begin_offset
        self.end_offset = end_offset

    def is_range(self, i):
        if self.begin_offset is not None and i < self.begin_offset:
            return False
        if self.end_offset is not None and self.end_offset <= i:
            return False
        return True

    def get_size(self):
        offset = 0
        count = 0
        for data_path in self.data_path_list:
            h = h5py.File(data_path, 'r')
            sz = h[self.div]['pid'].shape[0]
            if not self.begin_offset and not self.end_offset:
                offset += sz
                count += sz
                continue
            if self.begin_offset and offset + sz < self.begin_offset:
                offset += sz
                continue
            if self.end_offset and self.end_offset < offset:
                break
            for i in range(sz):
                if not self.is_range(offset + i):
                    continue
                count += 1
            offset += sz
        return count

    def get_each_class(self, h, i):
        b = h['bcateid'][i]
        m = h['mcateid'][i]
        s = h['scateid'][i]
        d = h['dcateid'][i]
        return b,m,s,d


    def generate(self) :
        offset = 0
        for data_path in self.data_path_list:
            h = h5py.File(data_path, 'r')[self.div]
            sz = h['pid'].shape[0]
            if self.begin_offset and offset + sz < self.begin_offset:
                offset += sz
                continue
            if self.end_offset and self.end_offset < offset:
                break

            left = max(0, self.begin_offset-offset)
            right = min(sz, self.end_offset-offset)

            dummy = {}
            for kind in ['pid', 'img_feat', 'bcateid', 'mcateid', 'scateid', 'dcateid', 'product', 'model', 'brand', 'maker'] :
                dummy[kind] = h[kind][left:right]

            for i in range(len(dummy['pid'])) :
                pid = dummy['pid'][i].decode('utf8')
                img_feat = dummy['img_feat'][i]
                b,m,s,d = dummy['bcateid'][i], dummy['mcateid'][i], dummy['scateid'][i], dummy['dcateid'][i]
                text = ' '.join([dummy[kind][i].decode('utf8') for kind in ['product', 'model', 'maker']])
                text_brand = dummy['brand'][i].decode('utf8')
                yield pid, img_feat, text, text_brand, b, m, s, d


            # for i in range(0, sz, 10000):
            #     if not self.is_range(offset + i):
            #         continue

            #     pid = h['pid'][i].decode('utf8')
            #     img_feat = h['img_feat'][i]
            #     b,m,s,d = self.get_each_class(h,i)
            #     text = ' '.join([h[kind][i].decode('utf8') for kind in ['product', 'model', 'brand', 'maker']])

            #     yield pid, img_feat, text, b, m, s, d
            offset += sz



def preprocessing(data):
    try:
        cls, data_path_list, div, out_path, begin_offset, end_offset, sequence = data
        data = cls()
        data.preprocessing(data_path_list, div, begin_offset, end_offset, out_path, sequence=sequence)
    except Exception:
        raise Exception("".join(traceback.format_exception(*sys.exc_info())))

class Data:
    # y_vocab_path = './data/y_vocab.cPickle' if six.PY2 else './data/y_vocab.py3.cPickle'
    tmp_chunk_tpl = 'tmp/base.chunk.%s'
    num_b = 57
    num_m = 552
    num_s = 3190-1
    num_d = 404-1
    n_img_feat = 2048

    def __init__(self):
        self.logger = get_logger('data')
        self.word_to_idx = self.load_word_to_idx()

        # make temporary save dir if not exists
        tempdir = os.path.dirname(self.tmp_chunk_tpl)
        if not os.path.isdir(tempdir) :
            os.makedirs(tempdir)


    def load_word_to_idx(self) :
        load_path = opt.word_path
        return pickle.load(open(load_path, 'rb'))

    def _split_data(self, data_path_list, div, chunk_size):
        total = 0
        for data_path in data_path_list:
            h = h5py.File(data_path, 'r')
            sz = h[div]['pid'].shape[0]
            total += sz
        chunks = [(i, min(i + chunk_size, total))
                  for i in range(0, total, chunk_size)]
        return chunks

    def _preprocessing(self, cls, data_path_list, div, chunk_size, sequence=False):
        chunk_offsets = self._split_data(data_path_list, div, chunk_size)
        # print(chunk_offsets)
        # return
        num_chunks = len(chunk_offsets)
        self.logger.info('split data into %d chunks' % (num_chunks))
        pool = Pool(opt.num_workers)
        try:
            pool.map_async(preprocessing, [(cls,
                                            data_path_list,
                                            div,
                                            self.tmp_chunk_tpl % cidx,
                                            begin,
                                            end,
                                            sequence)
                                           for cidx, (begin, end) in enumerate(chunk_offsets)]).get(99999)
            pool.close()
            pool.join()
        except KeyboardInterrupt:
            pool.terminate()
            pool.join()
            raise
        return num_chunks

    def text_to_idx(self, text, is_brand=False) :
        parse_words = wd.parse(text, is_brand)
        # idxs = list(set([self.word_to_idx.get(words) for words in parse_words if self.word_to_idx.get(words)!=None]))
        idxs = [self.word_to_idx.get(words) for words in parse_words if self.word_to_idx.get(words)!=None]

        # if is_brand :
        #     return idxs[:opt.max_len_brand]

        # return idxs[:opt.max_len]

        return idxs

    def preprocessing(self, data_path_list, div, begin_offset, end_offset, out_path, sequence):
        self.div = div
        reader = Reader(data_path_list, div, begin_offset, end_offset)
        rets = []

        for pid, img_feat, text, text_brand, b, m, s, d in reader.generate():
            uni_idxs = self.text_to_idx(text)
            uni_idxs_brand = self.text_to_idx(text_brand, is_brand=True)

            uni = uni_idxs_brand + uni_idxs
            
            # 추가시 sequence 유지 안함
            if not sequence : 
                uni = [x for x,c in Counter(uni).most_common(opt.max_len)]

            if np.max(img_feat) > 1000:
                continue
            # rets.append((pid, img_feat, uni_idxs, uni_idxs_brand, b, m, s, d))
            rets.append((pid, img_feat, uni[:opt.max_len], b, m, s, d))

        self.logger.info('sz=%s' % (len(rets)))
        open(out_path, 'wb').write(pickle.dumps(rets, 2))
        self.logger.info('%s ~ %s done. (size: %s)' % (begin_offset, end_offset, end_offset - begin_offset))


    def create_dataset(self, g, size):
        # shape = (size, opt.max_len)
        # g.create_dataset('uni', shape, chunks=True, dtype=np.int32)
        # g.create_dataset('w_uni', shape, chunks=True, dtype=np.float32)
        # g.create_dataset('cate', (size, num_classes), chunks=True, dtype=np.int32)

        g.create_dataset('pid', (size,), chunks=True, dtype='S12')
        g.create_dataset('img_feat', (size, self.n_img_feat), chunks=True, dtype=np.float32)
        g.create_dataset('uni', (size, opt.max_len), chunks=True, dtype=np.int32)
        # g.create_dataset('uni_brand', (size, opt.max_len_brand), chunks=True, dtype=np.int32)

        # for image classification
        g.create_dataset('b', (size,), chunks=True, dtype=np.int32)
        g.create_dataset('m', (size,), chunks=True, dtype=np.int32)
        g.create_dataset('s', (size,), chunks=True, dtype=np.int32)
        g.create_dataset('d', (size,), chunks=True, dtype=np.int32)


    def init_chunk(self, chunk_size):
        # chunk_shape = (chunk_size, opt.max_len)
        chunk = {}
        chunk['num'] = 0
        chunk['pid'] = []
        chunk['img_feat'] = np.zeros(shape=(chunk_size, self.n_img_feat), dtype=np.float32)
        chunk['uni'] = np.zeros(shape=(chunk_size, opt.max_len), dtype=np.int32)
        # chunk['uni_brand'] = np.zeros(shape=(chunk_size, opt.max_len_brand), dtype=np.int32)

        # for image classification
        chunk['b'] = np.zeros(shape=(chunk_size,), dtype=np.int32)
        chunk['m'] = np.zeros(shape=(chunk_size,), dtype=np.int32)
        chunk['s'] = np.zeros(shape=(chunk_size,), dtype=np.int32)
        chunk['d'] = np.zeros(shape=(chunk_size,), dtype=np.int32)

        return chunk

    def copy_chunk(self, dataset, chunk, offset, with_pid_field=False):
        num = chunk['num']
        dataset['img_feat'][offset:offset + num] = chunk['img_feat'][:num]
        dataset['uni'][offset:offset + num] = chunk['uni'][:num]
        # dataset['uni_brand'][offset:offset + num] = chunk['uni_brand'][:num]

        dataset['b'][offset:offset + num] = chunk['b'][:num]
        dataset['m'][offset:offset + num] = chunk['m'][:num]
        dataset['s'][offset:offset + num] = chunk['s'][:num]
        dataset['d'][offset:offset + num] = chunk['d'][:num]

        if with_pid_field:
            dataset['pid'][offset:offset + num] = chunk['pid'][:num]

    # def copy_bulk(self, A, B, offset, y_offset, with_pid_field=False):
    #     num = B['cate'].shape[0]
    #     y_num = B['cate'].shape[1]
    #     A['uni'][offset:offset + num, :] = B['uni'][:num]
    #     A['w_uni'][offset:offset + num, :] = B['w_uni'][:num]
    #     A['cate'][offset:offset + num, y_offset:y_offset + y_num] = B['cate'][:num]
    #     if with_pid_field:
    #         A['pid'][offset:offset + num] = B['pid'][:num]

    def get_train_indices(self, size, train_ratio):
        train_indices = np.random.rand(size) < train_ratio
        train_size = int(np.count_nonzero(train_indices))
        return train_indices, train_size

    def make_db(self, data_name, train_ratio=0.8, sequence=True):
        data_root = opt.rawdata_location
        if data_name == 'train':
            div = 'train'
            data_path_list = [os.path.join(data_root,filename) for filename in opt.train_data_list]
            output_path = opt.data_root + "train/data.h5py"

        elif data_name == 'dev':
            div = 'dev'
            data_path_list = [os.path.join(data_root,filename) for filename in opt.dev_data_list]
            output_path = opt.data_root + "dev/data.h5py"

        elif data_name == 'test':
            div = 'test'
            data_path_list = [os.path.join(data_root,filename) for filename in opt.test_data_list]
            output_path = opt.data_root + "test/data.h5py"

        else:
            assert False, '%s is not valid data name' % data_name

        all_train = train_ratio >= 1.0
        all_dev = train_ratio == 0.0

        np.random.seed(17)
        self.logger.info('make database from data(%s) with train_ratio(%s)' % (data_name, train_ratio))

        num_input_chunks = self._preprocessing(Data,
                                              data_path_list,
                                              div,
                                              chunk_size=opt.chunk_size, sequence=sequence)

        # num_input_chunks = 407

        # output_path = opt.data_path
        output_dir = os.path.dirname(output_path)
        if not os.path.isdir(output_dir):
            os.makedirs(output_dir)

        data_fout = h5py.File(output_path, 'w')

        reader = Reader(data_path_list, div, None, None)
        tmp_size = reader.get_size()
        train_indices, train_size = self.get_train_indices(tmp_size, train_ratio)

        dev_size = tmp_size - train_size
        if all_dev:
            train_size = 1 # 0으로 왜 안햇지?
            dev_size = tmp_size
        if all_train:
            dev_size = 1
            train_size = tmp_size

        train = data_fout.create_group('train')
        dev = data_fout.create_group('dev')
        self.create_dataset(train, train_size)
        self.create_dataset(dev, dev_size)
        self.logger.info('train_size ~ %s, dev_size ~ %s' % (train_size, dev_size))

        sample_idx = 0
        dataset = {'train': train, 'dev': dev}
        num_samples = {'train': 0, 'dev': 0}
        chunk_size = opt.db_chunk_size
        chunk = {'train': self.init_chunk(chunk_size),
                 'dev': self.init_chunk(chunk_size)}
        chunk_order = list(range(num_input_chunks))
        np.random.shuffle(chunk_order)
        for input_chunk_idx in chunk_order:
            path = os.path.join(self.tmp_chunk_tpl % input_chunk_idx)
            self.logger.info('processing %s ...' % path)
            data = list(enumerate(pickle.loads(open(path, 'rb').read())))
            np.random.shuffle(data)

            # for data_idx, (pid, img_feat, uni, uni_brand, b, m, s, d) in data:
            for data_idx, (pid, img_feat, uni, b, m, s, d) in data:
                is_train = train_indices[sample_idx + data_idx]
                if all_dev:
                    is_train = False
                if all_train:
                    is_train = True

                c = chunk['train'] if is_train else chunk['dev']
                idx = c['num']
                c['img_feat'][idx] = img_feat
                temp = np.zeros((opt.max_len,), dtype=np.int32)
                for i in range(len(uni)) :
                    temp[i] = uni[i]
                c['uni'][idx] = temp
                # temp = np.zeros((opt.max_len_brand,), dtype=np.int32)
                # for i in range(len(uni_brand)) :
                    # temp[i] = uni_brand[i]
                # c['uni_brand'][idx] = temp

                c['b'][idx] = b
                c['m'][idx] = m
                c['s'][idx] = s
                c['d'][idx] = d

                c['num'] += 1
                if not is_train:
                    c['pid'].append(np.string_(pid))
                for t in ['train', 'dev']:
                    if chunk[t]['num'] >= chunk_size:
                        self.copy_chunk(dataset[t], chunk[t], num_samples[t],
                                        with_pid_field=t == 'dev')
                        num_samples[t] += chunk[t]['num']
                        chunk[t] = self.init_chunk(chunk_size)
            sample_idx += len(data)
        for t in ['train', 'dev']:
            if chunk[t]['num'] > 0:
                self.copy_chunk(dataset[t], chunk[t], num_samples[t],
                                with_pid_field=t == 'dev')
                num_samples[t] += chunk[t]['num']

        for div in ['train', 'dev']:
            ds = dataset[div]
            size = num_samples[div]
            ds['img_feat'].resize((size, self.n_img_feat))
            ds['uni'].resize((size, opt.max_len))
            # ds['uni_brand'].resize((size, opt.max_len_brand))
            ds['b'].resize((size,))
            ds['m'].resize((size,))
            ds['s'].resize((size,))
            ds['d'].resize((size,))


        data_fout.close()


        self.logger.info('# of samples on train: %s' % num_samples['train'])
        self.logger.info('# of samples on dev: %s' % num_samples['dev'])
        self.logger.info('data: %s' % output_path)


if __name__ == '__main__':
    data = Data()
    fire.Fire({'make_db': data.make_db})
